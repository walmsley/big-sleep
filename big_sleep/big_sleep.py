...
class BigSleep(nn.Module):
    ...
    def forward(self, text_embed, return_loss = True):
        ...
        lat_loss_4 = self.loss_coefs[4] * (torch.mean(torch.max(torch.abs(img_grayscale_32-0.5), ones_32-0.65)**2) - (1-0.65)**2)#torch.sum(torch.gt(img_grayscale,ones_256-0.1))
